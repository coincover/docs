...
| `kid` | Key ID in the JWKS document. Use it when caching entries, not as a field on the key-assignment response. |
...
<Steps>
  <Step title="Load CoinCover's signing keys">
    Fetch the JWKS for the environment you called, or use a cached copy (see [Caching JWKS](#caching-jwks)). Keep only keys that match the assignment-signing constraints above.
  </Step>
  <Step title="Select the verification key">
    Use the eligible assignment-signing keys from that JWKS. The key-assignment response does not include a `kid`. Never verify with the `public_key` in the response — that is the recovery key, not CoinCover's attestation key.
  </Step>
...
- If you bound `key_fingerprint`, it equals the SHA-256 of the public-key **bytes** (hex-decode `public_key`, then hash), encoded as **lowercase hex** — not the SHA-256 of the hex string, and not uppercase hex.
...
Each JWKS endpoint returns `Cache-Control: max-age=600, must-revalidate` (ten minutes). Cache the document for that environment, indexed by the `kid` on each JWKS entry, rather than refetching on every key assignment. Keep a separate cache (or a composite key of environment + `kid`) for UAT and production.

During rotation the document can contain more than one key. Because the assignment response does not carry a `kid`, verify against the eligible assignment-signing keys in the cached set. If none of them verify, refresh the JWKS for that environment and retry once as the signing key may have rotated. Do not pin a single key forever.
...
| Hashing the hex string for `key_fingerprint` | Hex-decode `public_key` first, then SHA-256 the bytes. The digest is lowercase hex. |
| IEEE P1363 signature encoding | The signature is DER-encoded ECDSA. Configure your verifier accordingly. |
| Stale JWKS cache | If none of the cached keys verify, refresh the JWKS for that environment and retry once. |
