...
Store the `key_id` against your wallet record. The response also carries a `signature` over the `public_key`, signed by the generating enclave — verify it against CoinCover's published JWKS before relying on the key. See [Validating key signatures](/guides/validating-key-signatures). If verification fails, treat the response as untrusted and escalate.
