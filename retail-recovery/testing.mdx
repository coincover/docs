...
<Steps>
  ...
  <Step title="Generate a key">
    Confirm the key and signature are returned, and that the signature verifies against CoinCover's published JWKS — see [Validating key signatures](/guides/validating-key-signatures). A failed verification means the response should be treated as untrusted — exercise that path before going live.
  </Step>
