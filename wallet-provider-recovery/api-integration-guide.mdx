...
To verify the binding, check the signature over the exact `signed_payload` bytes against CoinCover's published JWKS, then confirm the bound values are the ones you expect. See [Validating key signatures](/guides/validating-key-signatures) for the verification process, including how to cache keys by `kid`.
